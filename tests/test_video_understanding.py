...
def _config(preset: str = "balanced", **overrides):  # noqa: ANN001, ANN202
    values = {
        ...
        "personification_video_visual_soft_limit": 96,
        "personification_video_visual_hard_limit": 128,
        "personification_video_max_scan_samples": 1800,
        "personification_video_contact_sheet_frames": 6,
    }
...
def test_three_minute_video_preset_targets_are_model_appropriate() -> None:
    assert video_understanding.resolve_video_frame_budget(180, _config("economy")).target_frames == 48
    assert video_understanding.resolve_video_frame_budget(180, _config("balanced")).target_frames == 72
    assert video_understanding.resolve_video_frame_budget(180, _config("quality")).target_frames == 96
...
def test_custom_budget_interpolates_and_never_exceeds_hard_limit() -> None:
    ...
    assert long.target_frames == 96  # custom also obeys the configured soft limit
...
def test_long_video_scan_rate_is_bounded_for_lightweight_server() -> None:
    ...
    assert budget.target_frames <= 128
...
def test_selection_combines_uniform_scene_and_subtitle_evidence() -> None:
    ...
    scores[111] = (1.0, 0.01)
    scores[333] = (0.01, 1.0)
    selected = video_understanding.select_storyboard_frames(scores, target_frames=72, scan_fps=4.0)
...
def test_default_contact_sheet_count_for_three_minutes_is_twelve() -> None:
    budget = video_understanding.resolve_video_frame_budget(180, _config("balanced"))
    assert budget.target_frames == 72
    assert budget.contact_sheet_frames == 6
    assert budget.target_frames // budget.contact_sheet_frames == 12
