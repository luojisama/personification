...
_GROUP_RULES: tuple[tuple[Callable[[str], bool], str], ...] = (
    ...
    (
        lambda k: k in {
            "meme_reply_probability",
            "slang_max_claims",
            "auto_understand_min_sources",
            "auto_use_min_sources",
            "auto_use_min_platforms",
            "claim_min_confidence",
            "semantic_equivalence_min_confidence",
            "reverify_after_days",
            "stale_after_days",
        },
        "原生 MCP",
    ...
    return {
        ...
        "item_schema": {
            ...
            "fields": [
                ...
                _ui_field("context_window_tokens", "上下文窗口 Token", "number", min=1, step=1, placeholder="留空使用 Provider 默认值"),
                _ui_field("max_input_tokens", "最大输入 Token", "number", min=1, step=1, placeholder="留空自动计算"),
                _ui_field("max_output_tokens", "最大输出 Token", "number", min=1, step=1, placeholder="留空使用模型默认值"),
                _ui_field("input_token_limit", "输入 Token 硬上限", "number", min=1, step=1, placeholder="留空不额外限制"),
...
_DETAILED_DESCRIPTIONS: dict[str, tuple[str, str]] = {
    ...
    "meme_reply_probability": (
        "已经决定回复后，允许自然带入一个 verified/manual_locked 低风险梗的概率（0~1）。不改变群消息触发概率，也不会额外主动开话题。",
        "0.18",
    ),
    "poke_probability": (
...
def _build_entries() -> list[ConfigEntry]:
    entries = [
        ...
        *[ConfigEntry(key="supplement_" + key, field_name="personification_supplement_" + key,
        ...
        ConfigEntry(
            key="media_protocol",
            field_name="personification_media_protocol",
            display_name="主模型音视频输入协议",
            value_type="str",
            default="auto",
            scope=GLOBAL_SCOPE,
            description="主模型音视频输入契约。Gemini API 类型允许受控探测但不代表已验证；代理网关必须显式选择真实协议。Gemini 原生小媒体 inline，大媒体仅官方 Google 端点可用 Files。",
            category="config",
            group="模型路由",
            choices=("auto", "none", "gemini_native", "openai_gemini_inline", "antigravity_native", "openai_qwen_omni", "openai_mimo_v25"),
        ...
        ConfigEntry(
            key="route_probe_daily_enabled",
            field_name="personification_route_probe_daily_enabled",
            display_name="每日能力探测",
            value_type="bool",
            default=True,
            scope=GLOBAL_SCOPE,
            description="每天以受控合成样本验证已启用路由的能力；不会发送 QQ 消息或执行社交写入。",
            category="config",
            group="模型路由",
            help_aliases=("每日探测", "route probe daily"),
            parser=_bool_parser,
        ),
        ConfigEntry(
            key="route_probe_daily_hour",
            field_name="personification_route_probe_daily_hour",
            display_name="每日探测小时",
            value_type="int",
            default=3,
            scope=GLOBAL_SCOPE,
            description="每日能力探测的本地启动小时（0 至 23）。",
            category="config",
            group="模型路由",
            min_value=0,
            max_value=23,
        ...
        ConfigEntry(
            key="route_probe_daily_minute",
            field_name="personification_route_probe_daily_minute",
            display_name="每日探测分钟",
            value_type="int",
            default=30,
            scope=GLOBAL_SCOPE,
            description="每日能力探测的本地启动分钟（0 至 59）。",
            category="config",
            group="模型路由",
            min_value=0,
            max_value=59,
        ...
        ConfigEntry(
            key="route_probe_item_timeout_seconds",
            field_name="personification_route_probe_item_timeout_seconds",
            display_name="单项探测超时",
            value_type="float",
            default=45.0,
            scope=GLOBAL_SCOPE,
            description="每项能力探测的最长秒数。超时会记录本次失败，不会抹去最近有效验证。",
            category="config",
            group="模型路由",
            min_value=1,
            max_value=45,
        ...
        ConfigEntry(
            key="route_probe_route_budget_seconds",
            field_name="personification_route_probe_route_budget_seconds",
            display_name="单路由探测预算",
            value_type="float",
            default=480.0,
            scope=GLOBAL_SCOPE,
            description="单个路由整轮探测的最长秒数，防止每日任务长期占用模型资源。",
            category="config",
            group="模型路由",
            min_value=10,
            max_value=480,
        ...
        ConfigEntry(
            key="route_probe_retention_days",
            field_name="personification_route_probe_retention_days",
            display_name="探测历史保留天数",
            value_type="int",
            default=90,
            scope=GLOBAL_SCOPE,
            description="能力探测操作历史的保留天数；最近有效能力证据会独立保留。",
            category="config",
            group="模型路由",
            min_value=1,
            max_value=3650,
        ...
        ConfigEntry(
            key="controlled_moderation_enabled",
            field_name="personification_controlled_moderation_enabled",
            display_name="受控禁言",
            value_type="bool",
            default=False,
            scope=GLOBAL_SCOPE,
            description="仅在授权群、Bot 具有管理员权限且完成两轮确认送达提醒后，允许受控禁言执行入口工作。",
            category="config",
            group="群管理",
            help_aliases=("受控禁言", "禁言总开关", "controlled moderation"),
            parser=_bool_parser,
        ),
        ConfigEntry(
            key="controlled_moderation_authorized_groups",
            field_name="personification_controlled_moderation_authorized_groups",
            display_name="受控禁言授权群",
            value_type="list",
            default=[],
            scope=GLOBAL_SCOPE,
            description="允许受控禁言的群 ID 列表。未列出的群即使 Bot 有管理员权限也不会执行。",
            category="config",
            group="群管理",
            help_aliases=("禁言授权群", "moderation groups"),
            parser=_json_array_parser,
        ),
        ConfigEntry(
        ...
        ConfigEntry(
            key="memory_retrieval_mode", field_name="personification_memory_retrieval_mode",
            display_name="记忆检索方式", value_type="str", default="algorithm_llm",
            scope=GLOBAL_SCOPE, description="算法与现有 LLM 为默认完整检索路径；hybrid_api 额外启用已配置的向量 API。",
            category="config", choices=("algorithm_llm", "hybrid_api"), parser=_str_parser,
        ),
        *[ConfigEntry(key=key, field_name="personification_"+key, display_name=label,
            value_type="int", default=default, scope=GLOBAL_SCOPE, description=description,
            category="config", group="记忆", parser=_int_parser) for key, label, default, description in (
                ("profile_batch_messages", "画像批处理消息数", 20, "达到该有效消息数时考虑刷新画像。"),
                ("profile_quiet_seconds", "画像静默等待秒数", 600, "会话静默后处理积累的画像证据。"),
                ("profile_scope_cooldown_seconds", "画像刷新间隔秒数", 600, "同一身份场景的最短刷新间隔。"),
                ("profile_daily_api_budget", "画像每日调用上限", 100, "后台画像 API 每日调用数量上限。"),
            )],
        ConfigEntry(
            key="memory_query_planning_enabled", field_name="personification_memory_query_planning_enabled",
            display_name="记忆查询规划", value_type="bool", default=True, scope=GLOBAL_SCOPE,
            description="在召回总预算内使用现有 LLM 扩展检索表达，失败时保留原始查询。",
            category="config", parser=_bool_parser,
        ),
        ConfigEntry(
            ...
            default=False,
            scope=GLOBAL_SCOPE,
            description="为 hybrid_api 检索配置向量服务；algorithm_llm 模式无需调用该服务。",
            category="config",
        ...
        ConfigEntry(
            ...
            category="config", group="记忆", help_aliases=("长上下文", "记忆回退", "memory context"), parser=_bool_parser,
        ),
        ConfigEntry(
            key="context_budget_enabled", field_name="personification_context_budget_enabled",
            display_name="上下文 Token 预算", value_type="bool", default=True, scope=GLOBAL_SCOPE,
            description="按当前 Provider 路由容量为历史、摘要和召回证据分配 Token 预算。关闭后不启用新的预算裁剪。",
            category="config", group="记忆", help_aliases=("上下文预算", "token预算", "context budget"), parser=_bool_parser,
        ),
        ConfigEntry(
            key="private_history_days", field_name="personification_private_history_days",
            display_name="私聊历史天数", value_type="int", default=DEFAULT_PRIVATE_HISTORY_DAYS, scope=GLOBAL_SCOPE,
            description="长上下文模式下，私聊原始消息最多保留并参与候选的天数。",
            category="config", group="记忆", min_value=1, max_value=365, help_aliases=("私聊保留天数",), parser=_int_parser,
        ),
        ConfigEntry(
            key="private_history_max_messages", field_name="personification_private_history_max_messages",
            display_name="私聊历史消息上限", value_type="int", default=DEFAULT_PRIVATE_HISTORY_MAX_MESSAGES, scope=GLOBAL_SCOPE,
            description="长上下文模式下，每个私聊会话最多保留的原始消息数。",
            category="config", group="记忆", min_value=100, max_value=50000, help_aliases=("私聊消息上限",), parser=_int_parser,
        ),
        ConfigEntry(
            key="group_history_days", field_name="personification_group_history_days",
            display_name="群聊历史天数", value_type="int", default=DEFAULT_GROUP_HISTORY_DAYS, scope=GLOBAL_SCOPE,
            description="长上下文模式下，群聊原始消息最多保留并参与候选的天数。",
            category="config", group="记忆", min_value=1, max_value=365, help_aliases=("群聊保留天数",), parser=_int_parser,
        ),
        ConfigEntry(
            key="group_history_max_messages", field_name="personification_group_history_max_messages",
            display_name="群聊历史消息上限", value_type="int", default=DEFAULT_GROUP_HISTORY_MAX_MESSAGES, scope=GLOBAL_SCOPE,
            description="长上下文模式下，每个群会话最多保留的原始消息数。",
            category="config", group="记忆", min_value=100, max_value=100000, help_aliases=("群聊消息上限",), parser=_int_parser,
        ),
        ConfigEntry(
            key="memory_retrieval_days", field_name="personification_memory_retrieval_days",
            display_name="自动召回时间范围", value_type="int", default=DEFAULT_MEMORY_RETRIEVAL_DAYS, scope=GLOBAL_SCOPE,
            description="自动召回只检索最近指定天数内的记忆；压缩产生的归档摘要仍由运行时按事务关联读取。",
            category="config", group="记忆", min_value=1, max_value=3650, help_aliases=("召回天数", "记忆时间范围"), parser=_int_parser,
        ),
        ConfigEntry(
            key="memory_auto_recall_timeout_seconds", field_name="personification_memory_auto_recall_timeout_seconds",
            display_name="自动召回超时（秒）", value_type="float", default=5.0, scope=GLOBAL_SCOPE,
            description="单轮自动召回最多等待的秒数；超时保持本轮可见诊断，不阻塞后续安全流程。",
            category="config", group="记忆", min_value=0.2, max_value=30, help_aliases=("召回超时",), parser=_float_parser,
        ),
        ConfigEntry(
            key="memory_auto_recall_candidate_limit", field_name="personification_memory_auto_recall_candidate_limit",
            display_name="自动召回候选数", value_type="int", default=32, scope=GLOBAL_SCOPE,
            description="每轮自动召回在重排前最多保留的候选记忆数量。",
            category="config", group="记忆", min_value=1, max_value=256, help_aliases=("召回候选",), parser=_int_parser,
        ),
        ConfigEntry(
            key="memory_auto_recall_inject_limit", field_name="personification_memory_auto_recall_inject_limit",
            display_name="自动注入记忆条数", value_type="int", default=12, scope=GLOBAL_SCOPE,
            description="每轮最终注入上下文的自动召回记忆上限。",
            category="config", group="记忆", min_value=0, max_value=64, help_aliases=("召回注入",), parser=_int_parser,
        ),
        ConfigEntry(
            key="session_compress_token_threshold", field_name="personification_session_compress_token_threshold",
            display_name="会话压缩 Token 阈值", value_type="int", default=0, scope=GLOBAL_SCOPE,
            description="超过该 Token 数时优先压缩旧消息；0 表示由当前路由的上下文预算压力自动决定。",
            category="config", group="记忆", min_value=0, max_value=1000000, help_aliases=("压缩token阈值",), parser=_int_parser, advanced=True,
        ),
        ConfigEntry(
            key="context_input_ratio", field_name="personification_context_input_ratio",
            display_name="上下文输入占比", value_type="float", default=DEFAULT_CONTEXT_INPUT_RATIO, scope=GLOBAL_SCOPE,
            description="路由可用上下文容量中可分配给输入的比例，其余容量留给模型输出与协议开销。",
            category="config", group="记忆", min_value=0.1, max_value=0.9, help_aliases=("输入预算比例",), parser=_float_parser, advanced=True,
        ),
        ConfigEntry(
            key="context_safety_margin_ratio", field_name="personification_context_safety_margin_ratio",
            display_name="上下文安全余量", value_type="float", default=DEFAULT_CONTEXT_SAFETY_MARGIN_RATIO, scope=GLOBAL_SCOPE,
            description="在路由上下文容量中预留的安全余量比例，避免边界 Token 估算导致请求超限。",
            category="config", group="记忆", min_value=0, max_value=0.3, help_aliases=("token安全余量",), parser=_float_parser, advanced=True,
        ),
        ConfigEntry(
        ...
        ConfigEntry(
            ...
            display_name="原生视频后备 Provider",
            value_type="str",
            default="",
            scope=GLOBAL_SCOPE,
            description="主模型不能直接读取视频时使用的原生音视频 Provider；Qwen-Omni 走百炼官方 HTTP API，Gemini 继续使用原生 generateContent。",
            category="config",
            help_aliases=("视频兜底供应商", "video_fallback_provider"),
            parser=_str_parser,
            choices=("", "auto", "qwen_omni", "gemini", "disabled"),
        ),
        ConfigEntry(
            key="video_fallback_workspace_id",
            field_name="personification_video_fallback_workspace_id",
            display_name="Qwen 百炼业务空间 ID",
            value_type="str",
            default="",
            scope=GLOBAL_SCOPE,
            description="Qwen-Omni 官方百炼业务空间 WorkspaceId；未填写自定义 Base URL 时据此生成北京地域 OpenAI-compatible 地址。",
            category="config",
            help_aliases=("视频百炼空间", "qwen workspace", "video_workspace_id"),
            parser=_str_parser,
        ),
        ConfigEntry(
            ...
            default="",
            scope=GLOBAL_SCOPE,
            description="原生音视频 Provider 的 Base URL。Qwen 可留空并填写 WorkspaceId；自定义地址必须使用 HTTPS。Gemini 留空使用官方地址。",
            category="config",
        ...
        ConfigEntry(
            ...
            default="",
            scope=GLOBAL_SCOPE,
            description="原生音视频模型名。Qwen 留空默认 qwen3.5-omni-plus；也可选 qwen3.5-omni-flash 或 150 秒以内的 qwen3-omni-flash。",
            category="config",
        ...
        ConfigEntry(
            key="meme_reply_probability",
            field_name="personification_meme_reply_probability",
            display_name="回复内玩梗概率",
            value_type="float",
            default=0.18,
            scope=GLOBAL_SCOPE,
            description="已经决定回复后，允许自然带入一个 verified/manual_locked 低风险梗的概率；每条回复最多一个，不提高原随机发言概率。",
            category="config",
            min_value=0.0,
            max_value=1.0,
            help_aliases=("玩梗概率", "meme probability"),
            parser=_float_parser,
        ),
        ConfigEntry(
            key="slang_max_claims",
            field_name="personification_slang_max_claims",
            display_name="单包最多黑话 Claim",
            value_type="int",
            default=20,
            scope=GLOBAL_SCOPE,
            description="一次社交内容包最多提取多少个独立黑话 claim；一篇内容可同时产生多个词义候选。",
            category="config",
            min_value=1,
            max_value=50,
            parser=_int_parser,
        ...
        ConfigEntry(
            key="auto_understand_min_sources",
            field_name="personification_auto_understand_min_sources",
            display_name="自动只理解最少来源",
            value_type="int",
            default=2,
            scope=GLOBAL_SCOPE,
            description="一致词义进入 understand_only 所需的最少独立内容簇；安全下限固定为 2。",
            category="config",
            min_value=2,
            max_value=20,
            parser=_int_parser,
        ...
        ConfigEntry(
            key="auto_use_min_sources",
            field_name="personification_auto_use_min_sources",
            display_name="自动可用最少来源",
            value_type="int",
            default=3,
            scope=GLOBAL_SCOPE,
            description="一致词义升级 verified 所需的最少独立内容簇；不得低于 2。",
            category="config",
            min_value=2,
            max_value=30,
            parser=_int_parser,
        ...
        ConfigEntry(
            key="auto_use_min_platforms",
            field_name="personification_auto_use_min_platforms",
            display_name="自动可用最少平台",
            value_type="int",
            default=2,
            scope=GLOBAL_SCOPE,
            description="一致词义升级 verified 所需的平台覆盖数；不同平台的搬运内容仍按一个来源簇计算。",
            category="config",
            min_value=2,
            max_value=4,
            parser=_int_parser,
        ...
        ConfigEntry(
            key="claim_min_confidence",
            field_name="personification_claim_min_confidence",
            display_name="Claim 最低置信度",
            value_type="float",
            default=0.72,
            scope=GLOBAL_SCOPE,
            description="低于该置信度的模型提取 claim 不进入自动确认状态机。",
            category="config",
            min_value=0.0,
            max_value=1.0,
            parser=_float_parser,
        ...
        ConfigEntry(
            key="semantic_equivalence_min_confidence",
            field_name="personification_semantic_equivalence_min_confidence",
            display_name="Sense 归一最低置信度",
            value_type="float",
            default=0.80,
            scope=GLOBAL_SCOPE,
            description="模型判断 same/compatible/conflict 时，达到该置信度才影响 sense 合并或冲突状态。",
            category="config",
            min_value=0.0,
            max_value=1.0,
            parser=_float_parser,
        ...
        ConfigEntry(
            key="reverify_after_days",
            field_name="personification_reverify_after_days",
            display_name="复核窗口（天）",
            value_type="int",
            default=30,
            scope=GLOBAL_SCOPE,
            description="verified 自动 sense 在多少天后进入复核窗口；不会在到期时直接删除。",
            category="config",
            min_value=1,
            max_value=365,
            parser=_int_parser,
        ...
        ConfigEntry(
            ...
            field_name="personification_stale_after_days",
            display_name="过期降级（天）",
            value_type="int",
            default=90,
            scope=GLOBAL_SCOPE,
            description="没有新鲜证据达到该天数后降为 stale 并停止主动使用；历史与证据仍保留。",
            category="config",
            min_value=2,
            max_value=730,
            parser=_int_parser,
        ...
        ConfigEntry(
            key="expression_enabled",
            field_name="personification_expression_enabled",
            display_name="表情总开关",
            value_type="bool",
            default=True,
            scope=GLOBAL_SCOPE,
            description="所有自动表情发送的总开关。关闭后本地、协议、QQ 收藏和 QQ 推荐来源都不会发送。",
            category="config",
            group="表情包",
            help_aliases=("表情总开关", "expression master"),
            parser=_bool_parser,
        ),
        ConfigEntry(
            key="local_expression_enabled",
            field_name="personification_local_expression_enabled",
            display_name="本地表情来源",
            value_type="bool",
            default=None,
            scope=GLOBAL_SCOPE,
            description="是否允许从本地表情库发送。未设置时继承旧版本地表情开关，便于平滑升级。",
            category="config",
            group="表情包",
            help_aliases=("本地表情", "local expression"),
            parser=_bool_parser,
        ),
        ConfigEntry(
            key="native_expression_enabled",
            field_name="personification_native_expression_enabled",
            display_name="协议表情来源",
            value_type="bool",
            default=None,
            scope=GLOBAL_SCOPE,
            description="是否允许发送平台协议表情。未设置时继承旧版表情配置。",
            category="config",
            group="表情包",
            help_aliases=("协议表情", "native expression"),
            parser=_bool_parser,
        ),
        ConfigEntry(
            key="qq_favorite_expression_enabled",
            field_name="personification_qq_favorite_expression_enabled",
            display_name="QQ 收藏表情来源",
            value_type="bool",
            default=None,
            scope=GLOBAL_SCOPE,
            description="是否允许使用 QQ 收藏表情。未设置时继承旧版 QQ 表情开关及收藏概率为零的关闭意图。",
            category="config",
            group="表情包",
            help_aliases=("QQ收藏表情", "favorite expression"),
            parser=_bool_parser,
        ),
        ConfigEntry(
            key="qq_recommended_expression_enabled",
            field_name="personification_qq_recommended_expression_enabled",
            display_name="QQ 推荐表情来源",
            value_type="bool",
            default=None,
            scope=GLOBAL_SCOPE,
            description="是否允许请求和发送 QQ 推荐表情。未设置时继承旧版 QQ 表情配置；显式开启可覆盖旧收藏概率的继承值。",
            category="config",
            group="表情包",
            help_aliases=("QQ推荐表情", "recommended expression"),
            parser=_bool_parser,
        ),
        ConfigEntry(
