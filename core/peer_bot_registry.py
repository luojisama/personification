...
PEER_BOT_REGISTRY_VERSION = 1

PeerBotStatus = Literal["candidate", "approved", "rejected"]
PeerBotSource = Literal["llm_observation", "onebot_metadata", "manual"]
PeerBotRisk = Literal["read", "write", "admin", "dangerous"]

BOT_STATUSES = frozenset({"candidate", "approved", "rejected"})
BOT_SOURCES = frozenset({"llm_observation", "onebot_metadata", "manual"})
...
def _normalize_group_document(raw: Any) -> dict[str, Any]:
    ...
    normalized["commands"] = (
        copy.deepcopy(doc.get("commands")) if isinstance(doc.get("commands"), dict) else {}
    )
...
def validate_command_template(
    ...
    *,
    ...
    template = str(full_template or "").strip()
