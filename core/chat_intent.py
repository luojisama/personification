...
async def infer_turn_semantic_frame_with_llm(
    ...
    *,
    ...
    prompt = (
        ...
        "先搞懂群友在聊什么再接话才更像真人。\n"
        "1b-1. 如果当前媒体 provenance 已明确提供可用图片/视频/音频，‘尚未分析’不等于‘无法查看’；"
        ...
        "只有在对方实际询问插件记忆机制、持久化存储、跨会话/跨群可用性、配置，或 bot 可访问哪些用户/群信息的能力范围时，"
        "才使用 plugin_question/capability，除非以后有对应 Tool 明确声明 runtime_capability。"
        "具体未知的用户事实、群内事实或历史内容本身不是插件能力问题；按内容判 explanation/lookup，并由后续 memory_need 或工具决定是否需要检索。"
        "如果对方只是让 bot 回顾、概述、澄清或确认当前消息和最近上下文已经提供的聊天内容（即使口语里说“记得”“聊天记录”“刚才聊了什么”），"
        "这是普通对话理解：按内容判 banter/explanation，evidence_policy 通常为 none，不要标 plugin_question、不要要求查询记忆或插件状态。"
        ...
        "不能只因为本轮是随机插话、没有 @bot 或结构化目标暂为 unclear 就机械静默。"
        "最新消息若是面向群聊的开放表达，且能沿最近同一话题自然推进半步，可以给 false；"
        "明确群友点对点对话、纯媒体/回声、私密话题或上下文明显不足时才优先 true。\n"
        "10. 群聊上下文中若标注“回复某人/提及某人/来源=其他插件输出/用户调用其它插件/命令/群聊旁观”，不要把它误认为用户正在要求 bot 解释内容；"
        "明确回复/提及其他群友时应提高插话谨慎度，必要时 recommend_silence=true；"
        "目标只是 unclear 时要继续结合最近话题、参与者和消息是否面向全群判断，不能等同于 target_others。\n"
        ...
        "语境不足时提高 ambiguity_level；只有随机插话、没有指向 bot，并且明显是别人的对话，"
        "或目标不明且现有上下文不足以自然参与时，才强制建议静默。\n"
