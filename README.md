...
| `同意登录 <code>` / `拒绝登录 <code>` | 管理员私聊 | 审批 WebUI 登录请求 |
...
- 验证码通过 Bot 私聊发送，5 分钟有效；也可以私聊回复 `同意登录 <code>` 或 `拒绝登录 <code>`。
- 首个设备可自动批准；之后默认需要已批准设备确认，可由 `personification_webui_require_device_approval` 调整。
...
- 登录限速、设备状态、危险写操作和管理员行为会记录审计日志。
- `personification_webui_expose_admin_list` 默认关闭，避免在登录页泄露管理员 QQ。
