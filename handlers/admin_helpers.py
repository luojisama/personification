...
def build_view_config_nodes(
    *,
    ...
    media_conf_str = _join_pairs(
        ...
        _pair("语音自动回复", _bool_text(plugin_config.personification_tts_auto_enabled), "开启后，Bot 会在部分场景由 LLM 决定是否改用语音回复。"),
        _pair("语音LLM决策", _bool_text(getattr(plugin_config, "personification_tts_llm_decision_enabled", True)), "合成前由 LLM 审查 voice/text/block 和违禁策略。"),
