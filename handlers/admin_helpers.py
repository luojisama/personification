...
def build_view_config_nodes(
    *,
    ...
    media_conf_str = _join_pairs(
        ...
        _pair("回复LLM审阅", _bool_text(getattr(plugin_config, "personification_response_review_enabled", False)), "发送前额外调用 LLM 审阅/改写，默认关闭。"),
        _pair("回复审阅模型", getattr(plugin_config, "personification_response_review_model_role", "review"), "intent/review/agent/sticker；agent 表示复用主模型。"),
        _pair("语音总开关", _bool_text(plugin_config.personification_tts_enabled), "控制整个插件是否允许使用 TTS 语音能力。"),
        _pair("语音自动回复", _bool_text(plugin_config.personification_tts_auto_enabled), "开启后允许自动语音；是否交给 LLM 决策取决于下方语音 LLM 决策开关。"),
        _pair("语音LLM决策", _bool_text(getattr(plugin_config, "personification_tts_llm_decision_enabled", False)), "合成前由 LLM 审查 voice/text/block 和违禁策略，默认关闭。"),
        _pair("语音审查模型", getattr(plugin_config, "personification_tts_llm_decision_model_role", "agent"), "intent/review/agent/sticker；agent 表示复用主模型。"),
