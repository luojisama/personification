...
function renderBuiltinPreviewDiagnostics(packet, result) {
  ...
  return `<div class="mcp-preview-diagnostics">
    ...
    ${semanticProcessing.extraction_elapsed_ms !== undefined ? `<div class="mcp-preview-summary">语义提取：<code>${escapeHtml(semanticProcessing.extraction_status || "unknown")}</code> / <code>${Number(semanticProcessing.extraction_elapsed_ms || 0).toLocaleString()}ms</code> · 学习归一：<code>${escapeHtml(semanticProcessing.learning_status || "unknown")}</code> / <code>${Number(semanticProcessing.learning_elapsed_ms || 0).toLocaleString()}ms</code> · 后台队列：<code>${Number(semanticProcessing.target_learning_queued || 0)}</code> · 目标 claims：<code>${Number(semanticProcessing.target_claim_count || 0)}</code></div>${Object.keys(extractionDiagnostics).length ? `<div class="mcp-preview-summary">证据输入：<code>${Number(extractionDiagnostics.input_item_count || 0)}</code> → 校验后 <code>${Number(extractionDiagnostics.validated_item_count || 0)}</code> → 送模 <code>${Number(extractionDiagnostics.selected_item_count || 0)}</code> · 模型调用：<code>${String(extractionDiagnostics.model_invoked === true)}</code> · 模型 claims：<code>${Number(extractionDiagnostics.model_claim_count || 0)}</code> → 有效 <code>${Number(extractionDiagnostics.validated_claim_count || 0)}</code></div>` : ""}` : ""}
