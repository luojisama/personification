...
function renderSkills() {
  ...
  if (state.skillsAvailable === false) return `${operationResult}<div class="card muted">tool_registry 未就绪</div>${renderRemoteSkillSources()}${renderMcpRegistry()}${renderMcpInstallations()}${renderMcpTools()}`;
...
function renderMcpRegistry() {
  ...
  const sourceOptions=sources.map(source=>`<option value="${escapeAttr(source.id)}" ${state.mcpSourceId===source.id?'selected':''}>${escapeHtml(source.name)}${source.preview?' · Preview':''}</option>`).join("");
  const rows=(state.mcpResults||[]).map(server=>`<tr><td><strong>${escapeHtml(server.title||server.name)}</strong><br><code>${escapeHtml(server.name||"")}</code></td><td class="muted">${escapeHtml(server.description||"")}</td><td>${escapeHtml(server.version||"")}</td><td>${Number(server.stdio_packages||0)} / ${Number(server.remote_count||0)}</td><td><button class="btn small" data-mcp-detail="${escapeAttr(server.name)}" data-mcp-source="${escapeAttr(server.source_id||state.mcpSourceId)}">详情</button></td></tr>`).join("");
  return `<div class="card"><div class="between"><div><h2>MCP 权威源</h2><p class="muted">内置 Official MCP Registry，并支持配置兼容 OpenAPI 的 HTTPS Registry。Registry 验证发布者命名空间，不代表 package 已经过安全审计。</p></div><span class="tag required">Preview metadata</span></div><div class="mcp-search-row"><select id="mcp-source-select">${sourceOptions}</select><input id="mcp-search-input" placeholder="搜索 MCP Server" value="${escapeAttr(state.mcpQuery||"")}"><button class="btn primary" data-mcp-search ${state.mcpBusy?'disabled':''}>搜索</button></div>${state.mcpDetail?renderMcpDetail():`<div class="table-wrap"><table><thead><tr><th>Server</th><th>说明</th><th>版本</th><th>stdio / remote</th><th></th></tr></thead><tbody>${rows||'<tr><td colspan="5" class="muted">输入名称或能力关键词搜索权威源。</td></tr>'}</tbody></table></div>`}</div>`;
}

function renderMcpDetail() {
  const detail=state.mcpDetail||{};const server=detail.server||{};const packages=detail.packages||[];
  const supported=packages.filter(item=>item.supported);
  if(!supported.length)return `<div class="mcp-detail"><button class="btn small" data-mcp-detail-close>返回结果</button><h3>${escapeHtml(server.title||server.name||"")}</h3><p>${escapeHtml(server.description||"")}</p><div class="alert info">该 Server 暂无可快捷安装的 npm/PyPI stdio package；Streamable HTTP 将在后续阶段支持。</div></div>`;
  let selected=supported.find(item=>Number(item.index)===Number(state.mcpPackageIndex));if(!selected){selected=supported[0];state.mcpPackageIndex=selected.index;}
  const packageOptions=supported.map(item=>`<option value="${Number(item.index)}" ${Number(item.index)===Number(selected.index)?'selected':''}>${escapeHtml(item.registry_type)} · ${escapeHtml(item.identifier)}@${escapeHtml(item.version)}</option>`).join("");
  const inputs=(selected.inputs||[]).map(input=>`<label class="mcp-input"><span>${escapeHtml(input.key)}${input.required?' *':''}</span><small>${escapeHtml(input.description||"")}</small>${input.choices&&input.choices.length?`<select data-mcp-install-input="${escapeAttr(input.key)}" data-mcp-secret="${input.secret?'true':'false'}">${input.choices.map(choice=>`<option value="${escapeAttr(choice)}">${escapeHtml(choice)}</option>`).join("")}</select>`:`<input data-mcp-install-input="${escapeAttr(input.key)}" data-mcp-secret="${input.secret?'true':'false'}" type="${input.secret?'password':'text'}" placeholder="${escapeAttr(input.default||'')}">`}</label>`).join("");
  return `<div class="mcp-detail"><button class="btn small" data-mcp-detail-close>返回结果</button><div class="between"><div><h3>${escapeHtml(server.title||server.name||"")}</h3><code>${escapeHtml(server.name||"")}</code></div><span class="tag">${escapeHtml(server.version||"")}</span></div><p>${escapeHtml(server.description||"")}</p><div class="alert info">快捷安装会下载并以 Bot 系统用户权限执行第三方 package；Secret 仅写入服务器受限文件，不会回显。预检后所有工具默认关闭，需逐项确认风险。</div><label>安装目标<select id="mcp-package-select">${packageOptions}</select></label><div class="mcp-input-grid">${inputs||'<p class="muted">该 package 不需要额外参数。</p>'}</div><label>工具名前缀（可选）<input id="mcp-prefix-input" placeholder="留空自动生成" value="${escapeAttr(state.mcpPrefix||"")}"></label><div class="row"><button class="btn primary" data-mcp-install ${state.mcpBusy?'disabled':''}>确认执行、预检并安装</button><a class="btn" href="${escapeAttr(safeHttpUrl((server.repository||{}).url||server.website_url||''))}" target="_blank" rel="noopener noreferrer">查看源码</a></div></div>`;
}

function renderMcpInstallations() {
  const cards=(state.mcpInstallations||[]).map(item=>{const tools=item.tools||[];const toolRows=tools.map(tool=>`<tr><td><strong>${escapeHtml(tool.registered_name||"")}</strong><br><span class="muted">${escapeHtml(tool.remote_name||"")}</span><br><small>${escapeHtml(tool.description||"")}</small></td><td>${tool.publisher_read_only?'<span class="tag">publisher: read-only</span>':'<span class="tag required">未知副作用</span>'}</td><td>${tool.enabled?'<span class="device-status approved">启用</span>':'<span class="device-status pending">关闭</span>'}</td><td><button class="btn small" data-mcp-tool-toggle="${escapeAttr(tool.remote_name)}" data-mcp-installation="${escapeAttr(item.installation_id)}" data-mcp-enabled="${tool.enabled?'false':'true'}" data-mcp-risk="${tool.publisher_read_only?'read':'unknown'}">${tool.enabled?'关闭':'启用'}</button></td></tr>`).join("");return `<section class="mcp-installation"><div class="between"><div><h3>${escapeHtml(item.server_title||item.server_name)}</h3><code>${escapeHtml(item.server_name||"")}@${escapeHtml(item.server_version||"")}</code></div><div class="row"><span class="device-status ${item.observed_status==='running'?'approved':'pending'}">${escapeHtml(item.observed_status||'stopped')}</span><button class="btn small" data-mcp-installation-toggle="${escapeAttr(item.installation_id)}" data-mcp-enabled="${item.desired_enabled?'false':'true'}">${item.desired_enabled?'停用':'启用'}</button><button class="btn small danger" data-mcp-delete="${escapeAttr(item.installation_id)}">删除</button></div></div><p class="muted">${escapeHtml(item.package_type||"")} · ${escapeHtml(item.package_identifier||"")} · prefix=${escapeHtml(item.name_prefix||"")}${item.secrets_configured?' · Secret 已配置':''}</p>${item.last_error?`<div class="alert err">runtime error: ${escapeHtml(item.last_error)}</div>`:""}<div class="table-wrap"><table><thead><tr><th>工具</th><th>风险依据</th><th>状态</th><th></th></tr></thead><tbody>${toolRows||'<tr><td colspan="4" class="muted">未发现工具</td></tr>'}</tbody></table></div></section>`;}).join("");
  return `<div class="card"><div class="between"><h2>MCP 安装</h2><button class="btn small" data-mcp-reload>重载 MCP</button></div>${cards||'<p class="muted">暂无托管 MCP 安装。</p>'}</div>`;
}

async function loadMcpInstallations(){const data=await api("/mcp/installations");state.mcpInstallations=data.installations||[];}
async function searchMcpRegistry(){state.mcpQuery=document.getElementById("mcp-search-input")?.value.trim()||"";state.mcpSourceId=document.getElementById("mcp-source-select")?.value||"official";state.mcpBusy=true;render();try{const data=await api("/mcp/search?"+new URLSearchParams({source_id:state.mcpSourceId,q:state.mcpQuery,limit:"30"}).toString());state.mcpResults=data.servers||[];state.mcpNextCursor=data.next_cursor||"";state.mcpDetail=null;}catch(e){const d=operationDiagnosticFromError(e,"MCP Registry 搜索失败");persistSkillOperationResult(d);alertFlash("err",d.title);}finally{state.mcpBusy=false;render();}}
async function openMcpDetail(name,sourceId){state.mcpBusy=true;render();try{state.mcpDetail=await api("/mcp/detail?"+new URLSearchParams({source_id:sourceId||state.mcpSourceId,name}).toString());const first=(state.mcpDetail.packages||[]).find(item=>item.supported);state.mcpPackageIndex=first?first.index:0;}catch(e){const d=operationDiagnosticFromError(e,"MCP Server 详情读取失败");persistSkillOperationResult(d);alertFlash("err",d.title);}finally{state.mcpBusy=false;render();}}
async function installMcpServer(){const detail=state.mcpDetail;if(!detail)return;const selected=(detail.packages||[]).find(item=>Number(item.index)===Number(state.mcpPackageIndex)&&item.supported);if(!selected)return;const inputs={};document.querySelectorAll("[data-mcp-install-input]").forEach(el=>{const key=el.getAttribute("data-mcp-install-input")||"";if(key&&el.value)inputs[key]=el.value;});const prefix=document.getElementById("mcp-prefix-input")?.value.trim()||"";if(!confirm("确认下载并以 Bot 系统用户权限执行这个第三方 MCP package？"))return;state.mcpBusy=true;render();try{const result=await api("/mcp/install",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({source_id:state.mcpSourceId,server_name:detail.server.name,package_index:Number(state.mcpPackageIndex),package_digest:selected.digest,inputs,name_prefix:prefix,confirm_execution:true})});persistSkillOperationResult(result);state.mcpDetail=null;state.mcpPrefix="";await loadMcpInstallations();await loadView();alertFlash("ok","MCP Server 已安装");}catch(e){const d=operationDiagnosticFromError(e,"MCP Server 安装失败");persistSkillOperationResult(d);alertFlash("err",d.title);}finally{state.mcpBusy=false;render();}}
async function toggleMcpInstallation(id,enabled){try{await api(`/mcp/installations/${encodeURIComponent(id)}/toggle`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({enabled})});await loadMcpInstallations();render();}catch(e){const d=operationDiagnosticFromError(e,"MCP Server 状态切换失败");persistSkillOperationResult(d);alertFlash("err",d.title);render();}}
async function toggleManagedMcpTool(id,name,enabled,risk){const confirmSideEffect=enabled;if(confirmSideEffect&&!confirm(`Registry 中的 ${risk==="read"?"readOnlyHint 仅是 publisher 声明，不能视为安全授权。":"工具副作用未知。"} 启用后将允许 Agent 调用，确认继续？`))return;try{await api(`/mcp/installations/${encodeURIComponent(id)}/tools/${encodeURIComponent(name)}/toggle`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({enabled,confirm_side_effect:confirmSideEffect})});await loadMcpInstallations();await loadView();render();}catch(e){const d=operationDiagnosticFromError(e,"MCP 工具状态切换失败");persistSkillOperationResult(d);alertFlash("err",d.title);render();}}
async function deleteMcpInstallation(id){if(!confirm("确认删除这个 MCP 安装、工具策略和 Secret？"))return;try{await api(`/mcp/installations/${encodeURIComponent(id)}`,{method:"DELETE",headers:{"content-type":"application/json"},body:JSON.stringify({confirm:"delete"})});await loadMcpInstallations();await loadView();render();}catch(e){const d=operationDiagnosticFromError(e,"MCP Server 删除失败");persistSkillOperationResult(d);alertFlash("err",d.title);render();}}

if(!window.__personificationMcpManagementEvents){window.__personificationMcpManagementEvents=true;document.addEventListener("change",event=>{const el=event.target;if(!(el instanceof Element))return;if(el.id==="mcp-package-select"){state.mcpPackageIndex=Number(el.value||0);render();}});document.addEventListener("click",event=>{const el=event.target instanceof Element?event.target.closest("[data-mcp-search],[data-mcp-detail],[data-mcp-detail-close],[data-mcp-install],[data-mcp-installation-toggle],[data-mcp-tool-toggle],[data-mcp-delete],[data-mcp-reload]"):null;if(!el)return;if(el.hasAttribute("data-mcp-search")){searchMcpRegistry();return;}if(el.hasAttribute("data-mcp-detail")){openMcpDetail(el.getAttribute("data-mcp-detail")||"",el.getAttribute("data-mcp-source")||"official");return;}if(el.hasAttribute("data-mcp-detail-close")){state.mcpDetail=null;render();return;}if(el.hasAttribute("data-mcp-install")){installMcpServer();return;}if(el.hasAttribute("data-mcp-installation-toggle")){toggleMcpInstallation(el.getAttribute("data-mcp-installation-toggle")||"",el.getAttribute("data-mcp-enabled")==="true");return;}if(el.hasAttribute("data-mcp-tool-toggle")){toggleManagedMcpTool(el.getAttribute("data-mcp-installation")||"",el.getAttribute("data-mcp-tool-toggle")||"",el.getAttribute("data-mcp-enabled")==="true",el.getAttribute("data-mcp-risk")||"unknown");return;}if(el.hasAttribute("data-mcp-delete")){deleteMcpInstallation(el.getAttribute("data-mcp-delete")||"");return;}if(el.hasAttribute("data-mcp-reload")){api("/mcp/reload",{method:"POST"}).then(()=>loadView()).then(render).catch(e=>alertFlash("err",e.message));}});}
