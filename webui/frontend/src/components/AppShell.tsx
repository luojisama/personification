...
import { NAVIGATION_GROUPS, NAVIGATION_ITEMS } from "../app/navigation";
...
function ShellContent() {
  ...
  const results = useMemo(() => {
    ...
    if (!needle) return [];
    return NAVIGATION_ITEMS.filter((item) => [item.label, ...item.aliases].join(" ").toLocaleLowerCase("zh-CN").includes(needle)).slice(0, 8);
  }, [search]);
  ...
  return (
    <>
      ...
      <div className="app-frame">
        <aside id="admin-navigation" className={`evidence-rail${drawerOpen ? " is-open" : ""}`} aria-label="管理台主导航">
          ...
          <div className="global-page-search">
            ...
            {results.length > 0 && (
              <div className="page-search-results" role="listbox" aria-label="页面搜索结果">
                {results.map((item) => <button key={item.path} type="button" role="option" onClick={() => visit(item.path)}><Icon name={item.icon} /><span>{item.label}</span><small>{item.aliases[0]}</small></button>)}
              </div>
            )}
          </div>
              ...
              <details className="nav-group" key={group.id} open>
                ...
                <div>
                  {group.items.map((item) => (
                    <NavLink key={item.path} to={item.path} end={item.path === "/"} onClick={() => setDrawerOpen(false)}>
                      <Icon name={item.icon} />
          ...
          <div className="rail-foot">
            <a className="legacy-entry" href="/personification/">进入旧版管理台</a>
            ...
            <StateBadge tone={realtime.state === "open" ? "ok" : realtime.state === "closed" ? "error" : "running"} raw={realtime.state}>
