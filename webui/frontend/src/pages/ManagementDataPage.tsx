...
function PersonaDetail({ section }: { section: string }) {
  ...
  return <div className="page-stack">
    ...
    {!userId ? <EmptyState code="persona_id_required">从画像列表选择用户，或输入 QQ ID。</EmptyState> : <QueryBoundary isPending={query.isPending} error={query.error}>{query.data && <>
      <div className="summary-grid"><Panel eyebrow="QQ / PUBLIC PROFILE" title={text(qq.nickname, userId)}><dl className="compact-kv"><dt>QQ ID</dt><dd><code>{userId}</code></dd><dt>签名</dt><dd>{text(qq.signature)}</dd><dt>所在地</dt><dd>{[qq.country, qq.province, qq.city].filter(Boolean).join(" / ") || "—"}</dd><dt>等级</dt><dd>{text(qq.level)}</dd><dt>更新时间</dt><dd>{formatDateTime(core.updated_at as string | number | null)}</dd></dl></Panel><Panel eyebrow="RELATION / FAVORABILITY" title={text(favorability.level, "未分级")}><dl className="compact-kv"><dt>分数</dt><dd>{text(favorability.score)}</dd><dt>作用域</dt><dd>{text(record(query.data.effective_scope).kind)}</dd><dt>有效声明</dt><dd>{Array.isArray(query.data.effective_claims) ? query.data.effective_claims.length : 0} 条</dd><dt>群画像</dt><dd>{Array.isArray(query.data.local_profiles) ? query.data.local_profiles.length : 0} 个群</dd></dl></Panel></div>
...
function GroupDetail({ section }: { section: string }) {
  const { params, set } = useUrlState();
  const groupId = params.get("group_id") ?? "";
  const apiSections = section === "knowledge" ? ["knowledge", "style", "memes"] : section === "members" ? ["personas", "aliases"] : ["schedule", "agent-state", "memes"];
  const detail = useQuery({ queryKey: ["group-business", groupId, apiSections.join("+")], queryFn: async ({ signal }) => Object.fromEntries(await Promise.all(apiSections.map(async (name) => [name, await resources.groupBusiness(groupId, name as Parameters<typeof resources.groupBusiness>[1], signal)]))), enabled: Boolean(groupId) });
