...
import { GroupQzoneAgentPanel } from "../components/GroupQzoneAgentPanel";
...
function GroupDetail({ section }: { section: string }) {
  ...
  const { params, set } = useUrlState();
  const groupId = params.get("group_id") ?? "";
  const specializedSection = section === "peer-bots" || section === "qzone-agent";
  const apiSections = section === "knowledge" ? ["knowledge", "style", "memes"] : section === "members" ? ["personas", "aliases"] : specializedSection ? [] : ["schedule", "agent-state", "memes"];
  const detail = useQuery({ queryKey: ["group-business", groupId, apiSections.join("+")], queryFn: async ({ signal }) => Object.fromEntries(await Promise.all(apiSections.map(async (name) => [name, await resources.groupBusiness(groupId, name as Parameters<typeof resources.groupBusiness>[1], signal)]))), enabled: Boolean(groupId && !specializedSection) });
  ...
  return <div className="page-stack">
    <PageHeader index={`群信息 / ${section === "knowledge" ? "知识与风格" : section === "members" ? "成员与别名" : section === "peer-bots" ? "Peer Bot 协作" : section === "qzone-agent" ? "空间互动" : "群详情"}`} title="群详情" description="成员画像、别名、群风格、知识、梗、Agent 状态、Peer Bot 协作、空间互动和计划均按当前群懒加载。" actions={<input value={groupId} onChange={(event) => set("group_id", event.target.value)} placeholder="群 ID" inputMode="numeric" />} />
    {!groupId ? <EmptyState code="group_id_required">从群列表选择群，或输入群 ID。</EmptyState> : section === "peer-bots" ? <GroupPeerBotsPanel groupId={groupId} botId={botId} /> : section === "qzone-agent" ? <GroupQzoneAgentPanel groupId={groupId} /> : <QueryBoundary isPending={detail.isPending} error={detail.error}>
