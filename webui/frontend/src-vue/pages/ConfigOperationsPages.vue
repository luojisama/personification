<template>
  <div class="page-stack">
          ...
          <Panel eyebrow="MODEL / MEDIA TOOLS" title="模型与媒体配置工具">
            <div class="dossier-actions">
              ...
              <button
                ...
                type="button"
                @click="requestConfirmation('speed')"
              >
                搜索速度测试
              </button>
              <button
                class="button button-secondary"
                type="button"
                @click="requestConfirmation('recommended')"
              >
                应用推荐配置
              </button>
            </div>
              ...
              <div class="config-entry-list">
                ...
                >
                  ...
                  <div class="config-editor-control">
                    <ConfigSchemaField
                      :item="item"
                      :model-value="getResolvedValue(item)"
                      :error="editorErrors[item.field_name]"
                      @update:model-value="updateDraft(item.field_name, $event)"
                      @update:error="setEditorError(item.field_name, $event)"
                    />

                    <button
                      v-if="item.field_name in draft"
                      type="button"
    ...
    <ConfirmDialog
      ...
      :dangerous="confirmationDetails.dangerous"
...
const configResultsRef = ref<HTMLElement | null>(null);
...
watch(
  [
    ...
    debouncedSearch,
    selectedGroup,
    () => activeFilters.value.modified,
    () => activeFilters.value.restart_required,
    () => activeFilters.value.hot_reloadable,
    () => activeFilters.value.advanced,
    () => activeFilters.value.secret,
    () => activeFilters.value.invalid,
  ],
  () => {
    if (configResultsRef.value) configResultsRef.value.scrollTop = 0;
  },
  { flush: "post" },
);
...
function setEditorError(field: string, message: string) {
  const next = { ...editorErrors.value };
  if (message) next[field] = message;
  else delete next[field];
  editorErrors.value = next;
}
function resetDraft(field: string) {
  const next = { ...draft.value };
  delete next[field];
  draft.value = next;
  const nextErrors = { ...editorErrors.value };
...
type ConfirmationAction = "save" | "speed" | "recommended" | "clear-logs";
const pendingConfirmation = ref<ConfirmationAction | null>(null);
const confirmationOpen = computed({
  get: () => pendingConfirmation.value !== null,
  set: (isOpen: boolean) => {
    if (!isOpen) pendingConfirmation.value = null;
  },
});
const confirmationDetails = computed(() => {
  switch (pendingConfirmation.value) {
    case "save":
      return {
        title: "原子保存配置草稿",
    ...
    case "recommended":
      return {
        title: "应用推荐配置",
        description: "将应用服务端推荐配置并写入配置文件。确认继续吗？",
        confirmLabel: "应用推荐",
        dangerous: true,
      };
    case "speed":
      return {
        title: "执行搜索速度测试",
        description: "将对已配置搜索引擎产生真实网络请求。确认继续吗？",
        confirmLabel: "开始测试",
        dangerous: false,
      };
    case "clear-logs":
      return {
        title: "清理插件管理日志",
        description: "该操作不可逆，只清理插件管理日志，不影响 Trace 数据库。",
        confirmLabel: "确认清理",
        dangerous: true,
      };
    default:
...
function confirmPendingAction() {
  ...
  if (action === "speed" || action === "recommended") {
    void executeConfigTool(action);
  }
}
