...
4. 点击“扫码/官方登录”。MCP 会优先使用本机正式 Chrome/Edge 打开独立官方窗口，并在短时轮询中读取官方二维码；没有系统浏览器时回退到 Playwright 自带 Chromium。
5. 在 WebUI 扫码。扫码后若平台要求手机确认、机器人验证、短信验证码、滑块或其它人工验证，只在已打开的官方窗口或官方 App 内完成；状态页会继续检查是否登录成功，但不会操作或绕过验证。
6. 平台状态变为 `ready` 后，再逐项授权 Agent 工具。
7. 使用检索预览读取一个真实词，确认封面、标题、正文/文案、评论/回复和能力矩阵符合预期。
...
当前登录页适配基线（2026-07-26）：B站二维码是官方 `Scan me!` 图片，小黑盒使用 `website-login__qr-canvas`，抖音使用官方“二维码”图片。MCP 同时保留兼容选择器，并通过二维码内容 revision 让 WebUI 在官方页面刷新二维码后取到新图，而不是继续显示浏览器缓存中的旧图。WebUI 使用带白色静区的 240px 近邻缩放展示原始 PNG，避免把小黑盒等较密二维码压缩到原来的 150px 后难以识别。
...
| `qr_expired` | 官方二维码已经失效 | 点击“重新获取二维码”，不要继续扫描旧图 |
...
2026-07-26 使用隔离临时 profile 对 B站、抖音和小黑盒官方页面做了未登录真实探针，三者均返回 `waiting_scan`、`qr_available=true`、`qr_revision=1`，随后已关闭窗口并删除临时 profile。这个探针证明当前二维码定位与延迟轮询有效，但不等于真实账号登录、扫码后验证或内容读取验收。
